...
class MfaAccount extends \plugin
{
    ...
    public $skipWrite;
    public $skipWrite_mfaRequiredByUser;
    ...
    public function execute()
    {
        ...
        /* If user has no tokens of type (must be in allowedTokenTypes) active, then we also need to set skipWrite */
        $this->skipWrite_mfaRequiredByUser = $this->skipWrite || !$this->containsActiveTokenSerials([], $this->allowedTokenTypes);
        ...
        foreach ($providedACLs as $name => $transl) {
            switch ($name) {
                ...
                case "mfaRequiredByRule":
                    $mfaSkipWrite = $this->skipWrite;
                    break;
                case "mfaRequiredByUser":
                    $mfaSkipWrite = $this->skipWrite_mfaRequiredByUser;
                    break;
                default:
                    $mfaSkipWrite = false;
            }
        ...
        if ($show_mfa_manager) {
            ...
            if (in_array_ics('add_token', $array_keys_post)) {
                ...
                switch ($mfaTokenAction) {
                    ...
                    case 'mfaTokenRemove':
                        if ($this->containsActiveTokenSerials(array($tokenSerial))) {
                ...
                switch ($mfaTokenAction) {
                    ...
                    case 'mfaTokenRevoke':       // FALLTHROUGH
                    case 'mfaTokenRemove':
                        if ($this->containsActiveTokenSerials($mfaTokenSerials)) {
                            // Prevent users from locking themselves out
                            msg_dialog::display(
                                _("Unsupported action"),
                                _("It is not supported to deactivate, revoke or remove your last token.") . "<br>" .
    ...
    /**
     * This function checks if a user has active tokens of given types.
     * @param array|null $tokenSerials
     * @param array|null $types
     * @return bool TRUE, if user has active tokens of either of the given types.
     */
    private function containsActiveTokenSerials($tokenSerials = [], $types = [])
    {
        $tokens = $this->utils->getTokensOfUser($this->getUid(), $this->userRealm);
        $activeTokenSerials = $this->utils->getActiveTokenSerials($tokens, $types);
        if ($tokenSerials) {
            return (count(array_diff($activeTokenSerials, $tokenSerials)) > 0);
        } else {
            return (count($activeTokenSerials) > 0);
        }
    }
    ...
    private function getMFAIntroTemplate()
    {
        ...
        $this->smarty->assign("mfaRequiredByUserDisabled", $this->skipWrite_mfaRequiredByUser);
    ...
    public function getUid()
    {
        ...
        // use logged-in user for roletabs
        if (is_object($this->parent) && ($this->parent instanceof roletabs)) {
            return $this->ui->uid;
        }

        // fall back to logged-in user if no parent
    ...
    public function check()
    {
        ...
        // If user has no tokens of type (must be in allowedTokenTypes) active,
        // then we must deny setting mfaRequiredByUser attribute
        $deny_mfaRequiredByUser = (in_array("by_user", $this->mfaRequired) && !$this->containsActiveTokenSerials([], $this->allowedTokenTypes));

        if ($deny_mfaRequiredByUser) {
            $message[] = msg_dialog::display(_("Error"), _("You need to add a multifactor method before you can enable this setting."), ERROR_DIALOG);
        }
